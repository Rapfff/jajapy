...
class PCTMCTestclass(unittest.TestCase):
	...
	def test_CTMC_state(var):
		...
		var.assertEqual(m.l(0,1,'c_red,p_red'),5.0)
		var.assertEqual(m.tau(0,1,'c_red,p_red'),1.0)
		var.assertEqual(m.tau(0,2,'yellow'),0.0)
		var.assertEqual(m.tau(2,3,'c_orange,p_green'),2.0/2.1)
		var.assertEqual(m.tau(2,4,'c_orange,p_green'),0.1/2.1)
		t = 1.0
		lkl = 2.1*exp(-2.1*t)
		var.assertEqual(m.lkl(2,t),lkl)
		var.assertEqual(m.lkl(0,-t),0.0)
		var.assertEqual(set(m.getAlphabet()),
						set(['c_red,p_red','c_green,p_red','c_orange,p_green','c_red,p_green','c_orange,p_red','init']))
		var.assertEqual(m.getLabel(2),'c_orange,p_green')
	...
	def test_CTMC_logLikelihood(var):
		m.instantiate(['p'],[1.0])
		set1 = Set([['init', 'c_green,p_red', 'c_orange,p_green', 'c_red,p_green', 'c_red,p_red', 'c_green,p_red']],[1])
		...
		set2 = Set([['init', 0.20149229043430208, 'c_green,p_red',
					0.8691033110137091, 'c_orange,p_green',
					0.40391041817651124, 'c_red,p_green', 
					17.822666563218853, 'c_red,p_red',
					0.05498832925297038, 'c_green,p_red'],
					['init', 0.237901319423925, 'c_green,p_red',
					6.906613946631418, 'c_orange,p_green',
					0.6742555535289178, 'c_red,p_green',
					5.788386627774087, 'c_red,p_red',
					0.019124343634252416, 'c_green,p_red']],[1,1])
