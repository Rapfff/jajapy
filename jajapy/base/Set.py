...
class Set:
	...
	def isEqual(self, set2) -> bool:
		...
		if len(self.sequences) != len(set2.sequences):
			return False
		if len(set(self.sequences).intersection(set(set2.sequences))) != len(set2.sequences):
			return False
...
def mergeSets(s1: Set, s2: Set) -> Set:
	"""
	...
	for i in range(len(s2.sequences)):
		if not s2.sequences[i] in s1.sequences:
			s1.sequences.append(s2.sequences[i])
			s1.times.append(s2.times[i])
		else:
			s1.times[s1.sequences.index(s2.sequences[i])] += s2.times[i]
	return s1
